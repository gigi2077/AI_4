...
export default async (request, context) => {
  ...
  try {
    ...
    if (!prompt) {
      return new Response(
        JSON.stringify({ error: "Prompt is missing from the request." }),
        { status: 400 }
      );
    }
    ...
    const geminiStream = await ai.models.generateContentStream({
      ...
      config: {
        ...
        // keep optional blocks closed correctly even if unused by the SDK
        thinkingConfig: {
          thinkingBudget: 0,
        },
      },
    ...
    return new Response(
      JSON.stringify({ error: `დაფიქსირდა შეცდომა: ${errorMessage}` }),
      { status: 500 }
    );
  }
};
